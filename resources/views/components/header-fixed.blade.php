...
<header class="fixed top-0 left-0 right-0 flex items-center justify-between gap-4 p-5 bg-white z-50">
    ...
    <div class="flex items-center space-x-4">
                ...
                @if ($currentSlug != 'segmentacao')
                    @foreach ($segmentacoes as $segmentacao)
                        @if ($currentSlug == $segmentacao->slug)
                            <div
                                class="block w-full bg-white text-black border border-black px-5 py-2 pr-5 rounded-full shadow leading-tight focus:outline-none focus:shadow-outline font-normal text-sm cursor-pointer hover:bg-gray-100 transition-colors duration-200 text-center">
        ...
        @if ($user)
            @if (count($parts) != 4 && count($parts) != 5 && count($parts) != 2 && count($parts) != 6)
                <button class="text-black font-normal hover:text-gray-600">
                    {{ $user->name ?? 'Usuário' }}
                </button>
                ...
                <form method="POST" action="{{ route('logout') }}" class="flex items-center">
                    @csrf
                    <button
                        class="flex items-center space-x-2 text-black hover:text-gray-700 transition-colors duration-150"
