...
<body class="min-h-screen flex flex-col bg-gray-50" x-data @open-modal.window="$store.modal.open($event.detail)"
    ...
    <div class="flex flex-1 container mx-auto">
        <aside
            class="inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transform md:relative md:translate-x-0 transition-all duration-300 ease-in-out -translate-x-full md:block overflow-y-auto">
            <div class="flex flex-col h-full">
                ...
                <nav class="flex-1 py-2 px-4 space-y-1">
                    ...
                    <a href="{{ url('/admin/segmentacao') }}"
                        class="flex items-center px-3 py-2 text-gray-600 rounded-lg hover:bg-gray-50 hover:text-gray-900 transition-colors duration-150 {{ str_contains($currentRoute, 'admin/segmentacao') ? 'bg-indigo-50 text-indigo-600' : '' }}">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
